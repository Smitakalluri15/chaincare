  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const configuredStart = import.meta.env.VITE_DONATION_START_BLOCK;
        if (runner?.getBlockNumber) {
          try {
            const currentBlock = await runner.getBlockNumber();
            // Bound the search to avoid querying millions of blocks if startBlock is 0/empty
            const startBlock = configuredStart
              ? Number(configuredStart)
              : Math.max(0, currentBlock - 50000);
            const step = 2000;
            const txMap = new Map();
            const maxChunks = 25;
            let chunks = 0;
            for (let s = startBlock; s <= currentBlock && chunks < maxChunks; s += step) {
              chunks++;
              ...
              events.forEach(e => {
                const id = Number(e.args?.donationId);
                if (Number.isFinite(id)) txMap.set(id, e.transactionHash);
              });
            }
            rows.forEach(r => { r.txHash = txMap.get(r.donationId) ?? null; });
          } catch { /* silently fail */ }
        }
